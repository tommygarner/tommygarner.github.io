...
This post documents a rigorous test of that question. We used 3-fold time-based cross-validation, 1000-resample bootstrap confidence intervals, and residual distribution tests to evaluate whether segmentation is worth the added complexity.
...
- **Stage 1 (Classification):** Predict whether any sales will occur in the next 7 days (binary). We use PR-AUC as the primary metric here since most snapshots have zero sales.
- **Stage 2 (Regression):** Given that sales are predicted to occur, estimate how many. We use a log-transformed target and evaluate in original ticket scale (end-to-end RMSE).

For segment models, we train a separate classifier and regressor on data from that category only.
...
The primary metric is **end-to-end RMSE in tickets** - the average prediction error across all test rows where sales were predicted to occur.
