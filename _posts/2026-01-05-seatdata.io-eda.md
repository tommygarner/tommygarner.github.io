...
After transformation, the distributions became approximately normal—much better for gradient-based optimization in neural networks.
...
Working with this real data, I quickly learned that missing data is inevitable. My approach: preserve as much signal as possible through intelligent imputation.

### 3.1 Venue Capacity (42% Missing)
...
I was still missing a lot of venue capacities and needed to be strategic now that I had lost my runway on my other source document. So, I made some assumptions on average venue sizes based off keywords in the name:
...
If I encoded Monday=1, Tuesday=2...Sunday=7, the model thinks Sunday (7) is "far" from Monday (1). But in reality, Sunday and Monday are just a day apart! The week is a **circle**, not a line.

### 4.2 Sine/Cosine Encoding

Think of the week as a clock face. Each day occupies a position on the circle:
- Sunday: 0° (BigQuery DAYOFWEEK returns 1 for Sunday)
- Monday: 51.4°
- Tuesday: 102.8°
- ...
- Saturday: 308.6°
...
My EDA discovered **5 days with Z-scores above 2.0** in absolute value—significant deviations from normal sales patterns.
...
*Figure 4: Interaction effect visualization—different slopes by category*
...
A single regression model struggles with this distribution—it's trying to simultaneously predict "no sales" and "how many sales."
...
- Compresses the scale: 2,803 → 7.94
...
This overlap is expected and desirable—we're testing on the **final snapshots** of events we've seen historically, simulating real deployment where we predict upcoming sales for known events.
...
**Key insight:** Past sales (`sales_total_7d_log` at 0.843) is by far the strongest predictor—momentum matters most in ticket sales.
...
job.result()
print(f"✅ Train Upload Complete: {len(df_train):,} rows")

# Upload test
job = client.load_table_from_dataframe(df_test, TEST_TABLE, job_config=job_config)
job.result()
print(f"✅ Test Upload Complete: {len(df_test):,} rows")
```

**Output:**
```
✅ Train Upload Complete: 5,114,513 rows
✅ Test Upload Complete: 58,022 rows
🎉 DONE. Proceed to Modeling.
```
