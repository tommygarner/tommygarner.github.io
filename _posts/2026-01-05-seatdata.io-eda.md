...
The "clipped" value would still be 2x the intended ceiling in log space. By clipping in raw space first, log transformation uses that 99th percentile as the ceiling and gives me a more managable range to work with.

### 2.4 Days to Event Outliers

Similarly, I found placeholder listings as well as legitimate listings for shows 2+ years in advance. So, I decided to filter my project to only look at **events happening within the next year** with a similar clipping process. However, it was just easier to do this in my SQL query bringing the data into my Python notebook using `WHERE days_to_event < 1000 AND event_date < '2020-01-01'`.

---

## 3. Temporal Feature Engineering

I found that ticket buying behavior is temporal and seasonal. Fans don't buy randomly but they buy based on **how close the event is** and **what day of the week it is** interestingly enough. I needed to encode these patterns mathematically.

### 3.1 Days-to-Event

The `days_to_event` feature (event_date - snapshot_date) captures how far out we're looking at an event. Values ranged from 0 to 338 days in my dataset. This raw feature goes directly into the model—scaling happens later (Section 10) after all features are engineered.
...
```python
model_df['price_spread_ratio'] = model_df['get_in'] / model_df['listings_median']
```
...
```python
model_df['inv_per_day'] = (
    model_df['listings_active'] /
    (model_df['days_to_event'] + 1)  # +1 to avoid division by zero
)
```
...
Inventory doesn't change randomly—sellers add and remove listings based on market signals. I calculated 7-day changes using lag features computed in SQL:
```python
model_df['listings_active_change_7d'] = (
    model_df['listings_active'] - model_df['listings_active_prev']
)
```
...
```python
model_df['sales_total_7d_log'] = np.log1p(model_df['sales_total_7d'])
```
...
```python
model_df['sales_total_change_7d'] = (
    model_df['sales_total_7d'] - model_df['sales_total_7d_prev']
)
```
...
Then I log-transformed:
```python
model_df['venue_capacity_log'] = np.log1p(model_df['venue_capacity'])
```
...
With all features engineered, I applied scaling as the final transformation step. This is critical for neural networks but doesn't affect tree-based models.

### 10.1 Why Scale at the End?

Scaling should happen **after** all feature engineering for two reasons:
1. **Prevent leakage**: Scaler must be fit on training data only
2. **Clean pipeline**: Log transforms and derived features are created first, then scaled together
...
This ensures the test set is scaled using statistics from training data only—no information leakage.

### 10.3 Scaled Feature Ranges

After scaling, all features had reasonable ranges centered around 0:
