...
- **Model Struggles with Extremes** - High-price concerts (<7 days out) show 2× higher error; 95th+ percentile prices lack training data
...
- Built interaction terms (bucket × days_to_event)
...
- 0.6% improvement for 4× inference cost
...
1. days_to_event: 23.1% ✓
2. sales_total_7d: 17.8% ✓
3. get_in_price: 14.2% ✓
...
### 2.5 Feature Interactions: Price × Time
...
- **30 days out:** Price change of $50 → 10 ticket swing
- **3 days out:** Price change of $50 → 40 ticket swing

Buyers are more price-sensitive when urgent. This validates creating `dte × price` interaction features.
...
- `dte × is_final_week` interaction terms
...
**On flagged events:**
- Return wider prediction intervals (±0.5 vs ±0.3 log-sales)
...
- "100 tickets (95% confident: 90-110)" ✓ Trust this
- "100 tickets (95% confident: 20-180)" ⚠️ Don't trust this

### 4.1 Ensemble Disagreement as Uncertainty Proxy

When the 4 base models (XGB, LGB, CAT, NN) **agree** → high confidence  
When they **disagree** → low confidence
```python
...
- Current: ±1.645 × ensemble_std
- Adjusted: ±1.8 × ensemble_std (for Concerts only)
...
**Uncertainty tiers:**
- **HIGH confidence:** ensemble_std < 0.15 (±45 tickets on 500-ticket event)
- **MEDIUM confidence:** ensemble_std 0.15-0.25 (±75 tickets)
- **LOW confidence:** ensemble_std > 0.25 (±125 tickets)
...
- **Unified model error:** ±52 tickets (0.2092 RMSE)
- **Mixed architecture error:** ±51 tickets (0.2034 RMSE)
- **Improvement:** 1-2 tickets per event

**Across 20,000 events annually:**
- **20,000-40,000 more accurate predictions**
- **Better inventory planning → Reduced stockouts/overstocks**
- **Optimized pricing → Revenue maximization**
...
6. If better: Deploy to staging → A/B test → Production
...
- Ridge → GB → XGBoost → Ensemble → Segments
...
**Systematic Approach**
- Clear progression: EDA → Features → Models → Ensemble → Segments → Production
...
**The Real Value:**
The final model achieves RMSE 0.2034, predicting ticket sales within ±51 tickets (90% confidence) for a typical 500-ticket event. That's good performance.
...
- Minor sports are heterogeneous (MLS ≠ MMA ≠ tennis)
...
Did it work? Follow-up article coming if the results are interesting... 😊
...
- [x] **Model trained and validated** (RMSE 0.2034, R² 0.9695)
