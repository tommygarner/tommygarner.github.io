...
The code above has some hyperparameters I can go ahead and introduce, since I'll show similar ones (and more) later. 
...
- `class_weight = 'balanced'` actually tells the RandomForest model that our data is imbalanced and to pay more attention to the minority class. This affects the loss function moreso
- `n_jobs=-1` and `verbose=0` are simply telling my computer how many cores to use to train this model and to refrain from printing any log statements

### 2.2 Results

**Test PR-AUC: 0.813**
**Test RMSE: ~0.70 (log units)**
