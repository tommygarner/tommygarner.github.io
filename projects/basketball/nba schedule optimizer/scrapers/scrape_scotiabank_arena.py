...
# Load the Scotiabank Arena URL from the Excel sheet
venue_events = pd.read_excel('venue_events.xlsx')
scotiabank_arena_url = venue_events.loc[venue_events['Venue'].str.contains('Scotiabank Arena', case=False, na=False), 'Website']
scotiabank_arena_url_value = scotiabank_arena_url.iloc[0] if not scotiabank_arena_url.empty else None

if not scotiabank_arena_url_value:
    print("Scotiabank Arena URL not found in the Excel sheet.")
    exit()
...
def get_current_month_year():
    ...
    try:
        header = driver.find_element(By.CSS_SELECTOR, '.tl-header .month_name').text
...
def navigate_to_next_month():
    ...
    try:
        next_button = driver.find_element(By.CSS_SELECTOR, '.tl-header .cal-next')
        next_button.click()
...
def extract_events_from_calendar():
    ...
    while current_month and current_month <= end_date_filter:
        try:
            ...
            day_elements = driver.find_elements(By.CSS_SELECTOR, ".tl-date.hasEvent")
...
# Start scraping events from Scotiabank Arena calendar
driver.get(scotiabank_arena_url_value)

events_data = extract_events_from_calendar()

# Save the scraped events to Scotiabank Arena Excel file
save_events_to_excel(events_data, 'scotiabank_arena_events.xlsx')
