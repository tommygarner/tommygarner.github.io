...
# Load the Kaseya Center URL from the Excel sheet
venue_events = pd.read_excel('venue_events.xlsx')
kaseya_center_url = venue_events.loc[venue_events['Venue'].str.contains('Kaseya Center', case=False, na=False), 'Website']
kaseya_center_url_value = kaseya_center_url.iloc[0] if not kaseya_center_url.empty else None

if not kaseya_center_url_value:
    print("Kaseya Center URL not found in the Excel sheet.")
    exit()
        ...
        try:
            ...
            for day_element in day_elements:
                try:
                    ...
                    #print(f"DEBUG: Error processing day element - {e}")
                    pass
        except Exception as e:
...
# Start scraping events from Kaseya Center calendar
driver.get(kaseya_center_url_value)

events_data = extract_events_from_calendar()

# Save the scraped events to Kaseya Center Excel file
save_events_to_excel(events_data, 'kaseya_center_events.xlsx')
