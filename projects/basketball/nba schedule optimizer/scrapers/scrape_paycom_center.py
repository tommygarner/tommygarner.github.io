...
# Load the Paycom Center URL from the Excel sheet
venue_events = pd.read_excel('venue_events.xlsx')
paycom_center_url = venue_events.loc[venue_events['Venue'].str.contains('Paycom Center', case=False, na=False), 'Website']
paycom_center_url_value = paycom_center_url.iloc[0] if not paycom_center_url.empty else None

if not paycom_center_url_value:
    print("Paycom Center URL not found in the Excel sheet.")
    exit()
...
# Start scraping events from Paycom Center calendar
driver.get(paycom_center_url_value)

events_data = extract_events_from_calendar()

# Save the scraped events to Paycom Center Excel file
save_events_to_excel(events_data, 'paycom_center_events.xlsx')
