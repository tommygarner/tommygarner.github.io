...
# Load the Target Center URL from the Excel sheet
venue_events = pd.read_excel('venue_events.xlsx')
target_center_url = venue_events.loc[venue_events['Venue'].str.contains('Target Center', case=False, na=False), 'Website']
target_center_url_value = target_center_url.iloc[0] if not target_center_url.empty else None

if not target_center_url_value:
    print("Target Center URL not found in the Excel sheet.")
    exit()
...
# Start scraping events from Target Center calendar
driver.get(target_center_url_value)

events_data = extract_events_from_calendar()

# Save the scraped events to Target Center Excel file
save_events_to_excel(events_data, 'target_center_events.xlsx')
